module Instana
  class Trace
    ...
    #
    def initialize(name, kvs = nil, incoming_context = nil, start_time = Time.now)
    ...
    # @param kvs [Hash] list of key values to be reported in the span
    #
    def finish(kvs = nil, end_time = Time.now)
    ...
    # @param kvs [Hash] list of key values to be reported in the span
    #
    def new_async_span(name, kvs = nil)
    ...
    #   from `log_async_entry`)
    #
    # FIXME: "argument must have default value" (span) but it shouldn't
    def end_async_span(kvs = nil, span = nil)
    ...
    # @param kvs [Hash] list of key values to be reported in the span
    #
    protected def configure_custom_span(span, name, kvs = nil)
      span ||= @current_span
    ...
    #
    # FIXME: "argument must have default value" (span) but it shouldn't
    protected def add_backtrace_to_span(bt, limit = nil, span = nil)
      frame_count = 0
      span[:stack] = [] of String
